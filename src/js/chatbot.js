...
// Make chatHistoryService globally available for other modules
window.chatHistoryService = chatHistoryService;

document.addEventListener('DOMContentLoaded', () => {
    console.log("DOM fully loaded and parsed");

    // Current user and conversation state
    let currentUser = null;
    let currentConversation = null;

    // Verify important elements exist in the DOM
    ...
    // Listen for custom event to force history updates
    document.addEventListener('forceHistoryUpdate', () => {
        console.log("Force update chat history triggered");
        updateChatHistory();
    });

    // Listen for continue conversation event from the popup
    document.addEventListener('continueConversation', (e) => {
        ...
        if (conversationId) {
            ...
            // Update the UI to show this is the active conversation
    ...
    // Listen for delete conversation event from the popup
    ...
    if (historyList) {
        ...
        // Attempt to resolve by finding sidebar and adding history list
        const sidebar = document.getElementById('sidebar');
        if (sidebar) {
            console.log("Attempting to fix missing history list...");
            // See if the element exists but with a different ID or class
    ...
    function updateChatHistory() {
        ...
        // Clear the loading message if exists
        ...
        chatHistoryService.getConversations(currentUser.uid)
            .then(conversations => {
                ...
                // Group conversations by topic ID for visual representation
                const topicGroups = {};
                ...
                // Create conversation items, grouped by topic
                conversations.forEach((conv, index) => {
                    ...
                    try {
                        ...
                        // Mark items from the same topic
                        ...
                        // Fetch conversation messages to show preview
                        chatHistoryService.getMessages(conv.id)
                            .then(messages => {
                                // Find the first user message and AI response (if available)
                            ...
                            .catch(error => {
                                ...
                                // Fallback to basic display if messages can't be loaded
                                convItem.innerHTML = `
                ...
                // Group conversations by topic and add visual indicators
    ...
    function groupConversationsByTopic(historyItems) {
        // First, add data attributes to all conversation items to identify them by topic
        const topicGroups = {};

        // Find all items and group by topic ID
        ...
        // Now mark items in the same topic group
        Object.entries(topicGroups).forEach(([topicId, group]) => {
            if (group.length > 1) {
                ...
                // Add a topic header for better visualization
                const topicHeader = document.createElement('div');
                topicHeader.className = 'topic-header';

                // Display the simplified topic ID (which is now already in a clean format)
                // For backward compatibility, show differently based on format
                ...
                // Add click handler to the topic header to select the first conversation
                topicHeader.addEventListener('click', () => {
                    // Find the first conversation item in this group
                ...
                // Insert the wrapper before the first item in the group
    ...
    // Function to handle conversation event listeners
    ...
    function startNewConversation() {
        ...
        // Store that user explicitly started a new conversation
        sessionStorage.setItem('startedNewChat', 'true');

        // Generate a new topic ID when starting a new conversation
        const newTopicId = chatHistoryService.startNewTopic();
        console.log("Started a new topic:", newTopicId);

        // Store topic ID in session storage for redundancy
        if (newTopicId) {
        ...
        // Use the Experience class method to start a new conversation
        if (experience) {
            ...
            // Update the chat history list to reflect the changes
            updateChatHistory();
    ...
    async function loadConversation(conversation) {
        ...
        // Clear the chat box and show loading indicator
        chatBox.innerHTML = '';
        showLoadingIndicator();

        try {
            // Use the Experience class method to load the conversation
            if (experience && conversation && conversation.id) {
                ...
                // Fetch the messages for this conversation and display them
                ...
                if (messages && messages.length > 0) {
                    ...
                    // Clear any existing messages first (just to be sure)
                    chatBox.innerHTML = '';

                    // Add messages to the chat UI
    ...
    async function createNewConversation(firstMessage) {
        try {
            ...
            // Ensure we have a valid topic ID (fallback to generating a new one as last resort)
            if (!topicId) {
            ...
            // Store the topic ID in the conversation object
    ...
    async function sendMessage(message) {
        ...
        try {
            // Check if we need to create a new conversation or continue an existing one
            const startedNewChat = sessionStorage.getItem('startedNewChat') === 'true';

            // If the user hasn't explicitly started a new chat and we have a current conversation,
            // continue with the current one, otherwise let the experience class handle it
            if (!startedNewChat && currentConversation) {
                console.log("Continuing with existing conversation:", currentConversation.id);

                // Ensure we have a topic ID even for existing conversations
                if (currentConversation.topic_id) {
                    // Make sure the topic ID is properly synchronized
            ...
            // Use the Experience class to send the message
            // This will automatically save to MySQL and update the conversation context
            ...
            // Check if we have a new conversation created
            ...
            if (currentConversationId && (!currentConversation || currentConversation.id !== currentConversationId)) {
                // Get the conversation details to update the UI
            ...
            updateChatHistory(); // Force refresh of chat history after saving AI response
    ...
    async function searchConversations(searchTerm) {
        ...
        try {
            ...
            // Use the Experience class's chatService to search the history
            ...
            // Create conversation items for search results
            searchResults.forEach(result => {
                ...
                // Format the date from the first message
                const date = new Date(result.messages[0].timestamp);
                const formattedDate = date.toLocaleDateString();

                // Create HTML for message excerpts with highlighted search terms
                ...
                convItem.querySelector('.history-content').addEventListener('click', () => {
                    // Get the full conversation from the server
                    loadConversation({ id: result.id, title: result.title });
                    clearSearch(); // Use the new function to clear search state
                });
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            // Store user info in session storage for access by ChatService
            const userInfo = {
                uid: user.uid,
                email: user.email,
            ...
            // Use a try-catch to ensure we handle any storage errors
            try {
            ...
            // Force update of chat history immediately after authentication
            setTimeout(() => {
                updateChatHistory();
            }, 500);

            // Ensure chatService has the current user
            if (experience && experience.chatService) {
                // Trigger a debug check to verify authentication
            ...
            // Force update of chat history to ensure conversations are loaded
            ...
            // Check if we're already in a conversation (after page reload)
            if (experience && experience.getCurrentConversationId()) {
                ...
                chatHistoryService.getConversations(user.uid).then(conversations => {
                    ...
                    if (!currentConversation) {
                        ...
                        // Load and display the conversation messages
